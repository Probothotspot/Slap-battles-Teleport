...
-- Адаптивный поиск портала (по Instance, точному пути или имени)
function AutoPortal.ResolvePortal(target)
    ...
    local targetName = tostring(target or "Teleport1")

    -- 1. Проверяем частый путь Lobby.Teleport1
    ...
    -- 3. Регистронезависимый поиск по всем объектам
...
-- Запуск авто-тача (постоянный цикл + отслеживание появления при спавне)
function AutoPortal.Start(config)
    config = config or {}
    local targetName = config.Target or "Teleport1"
    ...
    -- Моментальная реакция на спавн нового портала
    spawnConnection = Workspace.DescendantAdded:Connect(function(desc)
        ...
        if desc.Name == targetName or desc.Name:lower() == targetName:lower() then
            task.wait(0.05) -- микро-задержка на инициализацию хитбокса
